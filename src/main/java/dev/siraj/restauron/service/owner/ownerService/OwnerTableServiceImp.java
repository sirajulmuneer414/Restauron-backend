...
public class OwnerTableServiceImp implements OwnerTableService {
    ...
    @Transactional
    @Override
    public TableResponseDto updateTableStatus(String encryptedTableId, String status, Long ownerUserId)
        ...
        if (!table.getRestaurant().getOwner().getUser().getId().equals(ownerUserId)) {
            throw new AccessDeniedException("You are not authorized to update this table.");
        }
    ...
    // ---- Helper ----
    private TableResponseDto convertToDto(RestaurantTable item) {
